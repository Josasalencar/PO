...
for i in x:
    lista = geraLista(i)
    yMelhorCaso.append(
        timeit.timeit("insertionSort({})".format(sorted(lista)), setup="from __main__ import insertionSort", number=1))

    yPiorCaso.append(timeit.timeit("insertionSort({})".format(sorted(lista, reverse=True)),
                                   setup="from __main__ import insertionSort", number=1))

    yMedioCaso.append(
        timeit.timeit("insertionSort({})".format(lista), setup="from __main__ import insertionSort", number=1))
